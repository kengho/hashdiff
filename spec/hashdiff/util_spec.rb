...
describe HashDiff do
  ...
  it "should be able to tell similiar hash" do
    ...
    b = {'a' => 1, 'b' => 2, 'c' => 3, 'e' => 5}
    HashDiff.similar?(a, b).should be_truthy
    HashDiff.similar?(a, b, :similarity => 1).should be_falsey
  end
  ...
  it "should be able to tell similiar hash with values within tolerance" do
    ...
    HashDiff.similar?(a, b, :numeric_tolerance => 0.05).should be_truthy
    HashDiff.similar?(a, b).should be_falsey
  end

  it "should be able to tell numbers and strings" do
    HashDiff.similar?(1, 2).should be_falsey
    HashDiff.similar?("a", "b").should be_falsey
    HashDiff.similar?("a", [1, 2, 3]).should be_falsey
    HashDiff.similar?(1, {'a' => 1, 'b' => 2, 'c' => 3, 'e' => 5}).should be_falsey
  end
  ...
  it "should be able to tell true when similarity == 0.5" do
    ...
    HashDiff.similar?(a, b, :similarity => 0.5).should be_truthy
  end
  ...
  it "should be able to tell false when similarity == 0.5" do
    ...
    HashDiff.similar?(a, b, :similarity => 0.5).should be_falsey
  end

  describe '.compare_values' do
    it "should compare numeric values exactly when no tolerance" do
      expect(HashDiff.compare_values(10.004, 10.003)).to be_falsey
    end

    it "should allow tolerance with numeric values" do
      expect(HashDiff.compare_values(10.004, 10.003, :numeric_tolerance => 0.01)).to be_truthy
    end

    it "should compare other objects with or without tolerance" do
      expect(HashDiff.compare_values('hats', 'ninjas')).to be_falsey
      expect(HashDiff.compare_values('hats', 'ninjas', :numeric_tolerance => 0.01)).to be_falsey
      expect(HashDiff.compare_values('horse', 'horse')).to be_truthy
    end

    it 'should compare strings exactly by default' do
      expect(HashDiff.compare_values(' horse', 'horse')).to be_falsey
      expect(HashDiff.compare_values('horse', 'Horse')).to be_falsey
    end

    it 'should strip strings before comparing when requested' do
      expect(HashDiff.compare_values(' horse', 'horse', :strip => true)).to be_truthy
    end

    it "should ignore string case when requested" do
      expect(HashDiff.compare_values('horse', 'Horse', :case_insensitive => true)).to be_truthy
    end
